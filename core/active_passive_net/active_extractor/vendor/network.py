...
def custom_cropping_layer(input_layer, crop_from_beginning, crop_from_end, input_tensor_length, final_tensor_length):
    ...
    return Lambda(lambda x: K.reshape(x, (1, final_tensor_length, 1)))(clip_first_level)
...
def custom_constant_layer(const_size, name, np_constant=None):
    ...
    return Lambda(lambda x: K.reshape(x, const_fake_extender.shape))(const_fake_extender), const_fake_extender
...
def extract_semantic_tree_from_active_voice_branch(input_layer, roles, dual_roles, filler_len, max_depth):
    ...
    concatenate_verb = Concatenate(axis=1)([verb_extraction_output, tmp_reshaped_fake, tmp_reshaped_fake])
    ...
    tmp_reshaped_agentxr0_pxr1 = Lambda(lambda x: K.reshape(x, (1, filler_len * 2, 1)))(agentxr0_pxr1_output)
    concatenate_agentxr0_pxr1 = Concatenate(axis=1)([tmp_reshaped_fake, tmp_reshaped_agentxr0_pxr1])
    # TODO: why is there a constant 3?
