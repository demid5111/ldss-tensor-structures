...
def build_active_passive_network(roles, dual_roles, fillers, tree_shape):
    ...
    scalar_mul = tf.keras.layers.Lambda(lambda tensors: tensors[0] * tensors[1])([apnet_variable_input, classification_output])
    ...
    active_branch_input = tf.keras.layers.Lambda(lambda tensors: tensors[0] * tensors[1])([apnet_variable_input, is_active])
