...
from core.model_2_tuple import Model2Tuple, encode_model_2_tuple, FillerFactory, aggregate_model_tuples, \
    decode_model_2_tuple_tpr
...
def single_tpr_len(two_tuple_largest_scale_size) -> int:
    first_tuple = Model2Tuple(term_index=0, alpha=0,
                                                linguistic_scale_size=two_tuple_largest_scale_size)
    encoded_tuple, encoder = encode_model_2_tuple(first_tuple)
    flattened_encoded_tuple = flattenize_per_tensor_representation(encoded_tuple)
...
def log_generated_mta_samples(samples: List[np.ndarray], results, bits_per_number, numbers_quantity,
    ...
    for sample_index, sample in enumerate(to_check):
        ...
        model_tuples = [
            decode_model_2_tuple_tpr(i,
                                                       decoder=decoder,
                                                       model_2_tuple_has_weights=model_2_tuple_has_weights)[0]
            for i in tprs]
        res_tuple = decode_model_2_tuple_tpr(res_tpr, decoder=decoder,
                                                               model_2_tuple_has_weights=model_2_tuple_has_weights)[0]
        ...
        assert aggregate_model_tuples(model_tuples,
...
def generate_batch(numbers_quantity, batch_size, bits_per_number, bits_per_vector_for_inputs,
    ...
    for batch_index in range(batch_size):
        ...
        for _ in range(numbers_quantity):
            ...
            if mta_encoding in (MTATask.MTAEncodingType.full_no_weights, MTATask.MTAEncodingType.compact):
                weight = None
                first_tuple = Model2Tuple(term_index=random_index,
                                                            alpha=random_alpha,
                                                            linguistic_scale_size=two_tuple_largest_scale_size,
                                                            weight=weight)
        ...
        mta_result_tuple = aggregate_model_tuples(tuples, two_tuple_largest_scale_size)
    ...
    if mta_encoding == MTATask.MTAEncodingType.full or mta_encoding == MTATask.MTAEncodingType.full_no_weights:
        example_input, example_output = pack_with_full_mta_encoding(raw_dataset, bits_per_number,
                                                                    example_input,encoder)
...
def pack_with_compact_mta_encoding(raw_dataset, bits_per_number, example_input):
    """"
    ...
    return example_input, None #, example_output


def pack_with_full_mta_encoding(raw_dataset, bits_per_number, example_input, encoder):
    batch_tuples = []
    batch_result_tuples = []
...
def generate_batches(num_batches, batch_size, bits_per_vector, numbers_quantity,
    ...
    if mta_encoding == MTATask.MTAEncodingType.full:
        first_tuple = Model2Tuple(term_index=1, alpha=0.2,
                                                    linguistic_scale_size=two_tuple_largest_scale_size,
                                                    weight=.0)
        # 0. obtaining encoder and decoder networks for further re-use
        encoded_tuple, encoder = encode_model_2_tuple(first_tuple)
        flattened_encoded_tuple = flattenize_per_tensor_representation(encoded_tuple)
        decoded_tuple, decoder = decode_model_2_tuple_tpr(flattened_encoded_tuple)
        ...
        index, alpha, weight = FillerFactory._to_tpr_fillers(first_tuple)
